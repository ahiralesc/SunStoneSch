	...
	public void decide(Queue<Job> queue, Schedule schedule) {
		...
		if(!queue.isEmpty()) {
			...
			if (!possibleSlot.getAdvent().equals(startTime)) {
				...
				// Select size_j resources randomly
				// ResourceBundle selectedResources = possibleSlot.getResources().createSubSetWith(0); // Empty resource bundle
				ResourceBundle selectedResources = new ResourceBundleBitVectorImpl(super.getSite().getSiteInformation());
				ResourceBundle availableResources = possibleSlot.getResources();
	
				for(int j=1; j<=size; j++){
					int i = (int)(Math.random() * availableResources.size()-1);
