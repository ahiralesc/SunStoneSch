...
%             for j = 1 : number_of_columns_of_data
%                 errorbar(bar_first_position + (j - 1) * bar_separation, matrix_of_mean(1, j), matrix_of_std(1, j))
%             end
% 
%             set(gca,'xlim',[.5 1.5]);
%             set(gca,'XTickLabel','');
...
%            matrix_of_percent (2, :) = matrix_of_percent (1, :);
...
<<<<<<< .mine
plot(matrix_of_percent(1,:),'oK','LineWidth',1,'MarkerSize',5);
% plot(matrix_of_percent(1,:),'K'); %100
=======
%             %  Set the font name and size
%             set(gca, 'FontName', 'Times New Roman');
%             set(gca, 'FontSize', 10);
...
if(total_number_of_graphics == total_number_of_system_criterion_list)
    ...
    [matrix_for_rank_sorted,sorted_index] = sort(matrix_for_rank',1);

    matrix_for_rank_sorted = matrix_for_rank_sorted';
    ...
    errorbar(matrix_for_rank_sorted(1,:),matrix_for_rank_sorted(3,:),'oK','LineWidth',1,'MarkerSize',5);
    % errorbar(matrix_for_rank_sorted(1,:),matrix_for_rank_sorted(3,:),'K');

    %                plot(matrix_for_rank_sorted(1,:),'K'); %100
    %                plot(matrix_for_rank(1,:),'K'); %100
    ...
    title({'Average Performance Degradation' 'Ranking'});
    ylabel('PERCENT');
    xlabel('');
    ...
    %%%%%%
%     file_exists = fopen('results_for_rank_overall.mat', 'r');
% 
%     if(file_exists==-1)
%         matrix_for_rank_all(1,:) = matrix_for_rank(1,:);
%         save results_for_rank_overall matrix_for_rank_all
%         mrows=0;
%     else
%         fclose('all')
%         load results_for_rank_overall
%         [mrows,mcols] = size(matrix_for_rank_all);
%         matrix_for_rank_all(mrows+1,:) = matrix_for_rank(1,:);
%         save results_for_rank_overall matrix_for_rank_all
%     end
% 
%     figure(total_number_of_graphics+2)
%     for k=1:mrows+1
%         plot(matrix_for_rank_all(k,:),'K')
%         hold on
% %     end
% 
%     xtick_positions = 1 : number_of_columns_of_data;
